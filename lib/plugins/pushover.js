...
function init (env, ctx) {
  ...
  pushover.send = function wrapSend (notify, callback) {
    ...
    function prepareMessage() {
      ...
      console.info('send() pushoverAPI.emergencyPriority=' + pushoverAPI.emergencyPriority);
...
function setupPushover (env) {
  ...
  var emergencyPriority = !(env.extendedSettings && env.extendedSettings.pushover && !env.extendedSettings.pushover.emergencyPriority);
  ...
  if (apiToken && (userKeys.length > 0 || alarmKeys.length > 0 || announcementKeys.length > 0)) {
    ...
    console.info('setupPushover() pushoverAPI.emergencyPriority=' + pushoverAPI.emergencyPriority);
